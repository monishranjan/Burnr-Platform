...
export default function InboxPage() {
  ...
  const fetchInbox = useCallback(async () => {
    try {
      setError(null);
      const res = await fetch(`${API_BASE_URL}/emails/${id}/inbox`);
      if (!res.ok) {
  ...
  const fetchEmailData = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        throw new Error(`Failed to fetch email metadata: ${res.status} ${res.statusText}`);
      }
  ...
  return (
    <main className="min-h-screen flex flex-col text-text font-burnr items-center bg-animated-gradient">
      <Header />
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className="glass-card p-6 rounded-2xl flex flex-col md:flex-row justify-between gap-6 transition-transform hover:-translate-y-1"
              >
                <div>
